...
The supported environments are `apb` (A Plus B), `ts` (Thompson Sampling), and `multibss` (Multi BSS).

### start environment
In the second terminal type following command to start the ns-3 based environment:
```
python3 start_env_ns3_<environment-name>.py
```

### start client
In the third terminal, type the following command to start the client:
```
python3 start_client_<environment-name>.py
```
