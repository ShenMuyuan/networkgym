...
class Adapter(network_gym_client.adapter.Adapter):
    ...
    def get_action_space(self):
        ...
        # min: -82 dBm, max: -62 dBm
        return spaces.MultiDiscrete(np.array([21]), start=np.array([-82]), seed=42, dtype=np.int32)
    ...
    def get_observation(self, df):
        ...
        for _, row in df.iterrows():
            ids = row['id']
            values = row['value']
            if row['source'] == 'MultiBss' and row['name'] == 'Cpp2Py::RxPowerDbmMatrix':
            ...
            elif row['source'] == 'MultiBss' and row['name'] == 'Cpp2Py::NodeX':
                nodeLoc[ids, 0] = values
            elif row['source'] == 'MultiBss' and row['name'] == 'Cpp2Py::NodeY':
                nodeLoc[ids, 1] = values
            elif row['source'] == 'MultiBss' and row['name'] == 'Cpp2Py::McsIndex':
                mcsIndex[ids] = values
            elif row['source'] == 'MultiBss' and row['name'] == 'Cpp2Py::UplinkThptMbps':
                ulThpt[ids] = values
            else:
                id = ids[0]
                value = values[0]
                if row['source'] == 'MultiBss' and row['name'] == 'Cpp2Py::AccessDelayMs':
    ...
    def get_policy(self, action):
        ...
        policy['id'] = [0]
        policy["name"] = "Py2Cpp::CcaNew"
        policy["value"] = action.tolist()

        print('Action --> ' + str(policy))
        self.action_history.append(policy["value"][0])
        return policy
    ...
    def make_table(self, n=20):
        ...
        actions = np.full((n, ), np.nan)
        for i in range(min(n, len(self.action_history))):
            actions[i] = self.action_history[-i-1]
        ...
        table.add_column("new CCA (dBm)")

        for i in range(n):
            table.add_row("%.0f" % steps[i], "%.2f" % total_thpts[i], "%.2f" % vr_thpts[i], "%.2f" % vr_delays[i], "%.2f" % rewards[i], "%.0f" % actions[i])
