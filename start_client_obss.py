...
n_action = env.adapter.get_action_space().nvec[0] * env.adapter.get_action_space().nvec[1]
...
for step in range(num_steps):
    ...
    action_obsspd = -82 + np.floor(action.cpu().numpy()[0][0] / env.adapter.get_action_space().nvec[1])
    action_txpower = 16 + action.cpu().numpy()[0][0] % env.adapter.get_action_space().nvec[1]
    obs, reward, terminated, truncated, info = env.step(np.array([action_obsspd, action_txpower]))
